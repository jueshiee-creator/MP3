...
# 歌词搜索下载
——2020/4/17——
# 对歌词居中进行了改进，改进了bg网易云红，扩充改进了播放模式的选择
——2020/4/9——
# 对音量滑块进一步修改(音量scale显示时可在scale or 音量键上滚动)
——2020/4/4——
# 添加滚轮,songs
——2020/3/24——
# 对搜索功能添加,对界面稍作修改(新添背景)

@ -*- coding:utf-8 -*-
@Name   : 音乐播放器 player v1.6.1 
@Author : jue
@Time   : 2020/3/24
@Note   : play musics
"""
...
def music_get():
    a_thread = threading.Thread(target=music_download, daemon=True)
    a_thread.start()
...
def play_options():
    ...
    if music_list:
        ...
        if not pygame.mixer.music.get_busy():
            ...
            musicName.set(nextMusic.split(' - (')[0].split('.mp3')[0])
...
def play():
    """using for creating a playing thread"""
...
# 菜单栏---------------------------------------------
def introduce():
    msg.showinfo('v1.6.1版本功能介绍', 
    message= '在music player v1.6.1中功能不算太全,\n'
        '但能满足基本使用需求,播放| |随机\n'
        '播放等等，能控制进度条，其在功能帮助\n'
        '中详细说明。经测试，不连续点击至崩\n'
        ',能正常平稳运行,未遇到非常严重的bug\n'
...
def caidan(root, o1):
    ...
    submenu1_1.add_command(label='music_player v1.6.1')
