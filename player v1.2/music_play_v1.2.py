"""
player v1.2 2020/3/13
player v1.2 有播放缺陷, 功能也不完善"""
from tkinter import *
import pygame
import os, time
import threading
...
def order_play():
    ...
    if not pygame.mixer.music.get_busy():
        ...
        # 获取music长度
        # audio=MP3(folder_path+'\\'+nextMusic)
        ...
        # time_from = jdt.get()
        time_ = time_ + 0.1
        time_to = int(time_ / float(music_long) *100)
        #time_ = (pygame.mixer.music.get_pos()/1000)# +time_add
        musicTime.set('已播放 {:0>2d}:{:0>2d}'.format(int(time_//60),int(time_%60)))
        jdt.set(time_to)
...
def select_play():
    ...
    if not pygame.mixer.music.get_busy():
        ...
        time.sleep(0.1)
        time_ = (pygame.mixer.music.get_pos()/1000)
        musicTime.set('已播放 {:0>2d}:{:0>2d}'.format(int(time_//60),int(time_%60)))
...
def progress_bar(value=0):
    ...
    #     time_add = float((time_to-time_from)/100*m)
    # else:
    #     time_add = time_jd
...
jdt = Scale(root, from_=0, to=100, orient="horizontal", length=240,repeatinterval=1, command=progress_bar, label="进度")# variable=time_var,
