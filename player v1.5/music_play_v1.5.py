...
def order_play():
    ...
    if music_list:
        ...
        if not pygame.mixer.music.get_busy():
            ...
            global time_
            time_ = 0
            # if previous_index != current_index:  # 让滑块自动跳转到0的情况，换首歌=0
            #     time_ = 0
...
def volume_popup_click():
    """音量滑块弹出"""
    s.grid(row=2, column=7, rowspan=3, sticky=S, padx=(90,0),pady=(0,30)) 
...
# 菜单栏---------------------------------------------
def introduce():
    msg.showinfo('v1.5版本功能介绍', 
    message= '在music player v1.5中功能不算太全,\n'
        '但能满足基本使用需求,播放暂停随机\n'
        '播放等等，能控制进度条，其在功能帮助\n'
        '中详细说明。经测试，不连续点击至崩\n'
        ',能正常平稳运行,未遇到非常严重的bug\n'
...
def caidan(root, o1):
    ...
    submenu1_1.add_command(label='music_player v1.5')
    ...
    submenu3_1.add_command(label='网易云红', command=bg5_set)
    ...
    menu2.add_command(label="添加", command=add_playlist)
    def popup(event):
...
if __name__ == "__main__":
    ...
    #歌单
    o1=Listbox(root,width=25, heigh=23, font=("楷书", 15), bd=3, selectmode=BROWSE,bg="#B4F4CB") # 
    ...
    entry1.grid(row=0,rowspan=1, column=5, columnspan=1,sticky=W) # 只能写入
    # 搜索下载
    search_file = Button(root, text="选择文件/播放", command=alone_play, width=10, bg="#8DFCDD")
    search_file.grid(row=1,rowspan=1, column=4, columnspan=1,sticky=E)
    entry2 = Entry(root, text=pathn, width=25, bd=2)
    entry2.grid(row=1,rowspan=1, column=5, columnspan=1,sticky=W) # 只能写入
    ...
    # 音量弹出键(点击)
    volume_popup = Button(root, text='音量', bg="#8DFCDD", width=5, command=volume_popup_click)
    ...
    music_name = Label(root, textvariable=musicName, width=25,heigh=1, font=("楷书", 12),
    bd=3,bg="#8DFCDD")  # 
    music_name.grid(row=0, rowspan=1, column=7, columnspan=3,padx=(32,0))
    musicTime = StringVar(value='')
    music_time = Label(root, textvariable=musicTime, width=22,heigh=1, font=("Helvetica", 12),
    bg="#8DFCDD",bd=2, )
    music_time.grid(row=1, rowspan=1, column=7, columnspan=2,padx=(32,0))
    musicTime.set('00:00')
    ...
    # songs listbox
    songs = Listbox(root, bg="#85F0AC", width=105, heigh=27, bd=4)
    songs.grid(row=2, rowspan=1, column=4, columnspan=6)
